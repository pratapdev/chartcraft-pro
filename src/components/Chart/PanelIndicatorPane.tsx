...
export const PanelIndicatorPane: React.FC<Props> = ({ indicator, panelIndex, subCount = 1 }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const chartRef = useRef<IChartApi | null>(null);
  const seriesRefs = useRef<ISeriesApi<'Line' | 'Histogram'>[]>([]);

  const candles = useMultiPanelStore((s) => s.panels[panelIndex]?.candles ?? []);
  ...
  // Dynamic default height: smaller when more sub-indicators share the space
  const defaultH = subCount >= 3 ? 55 : subCount >= 2 ? 65 : 75;
  const [height, setHeight] = useState(defaultH);
  const isDragging = useRef(false);
  const startY = useRef(0);
  const startHeight = useRef(defaultH);
  ...
  useEffect(() => {
    ...
    if (indicator.type === 'RSI') {
      ...
      series.push(chart.addLineSeries({ color: 'rgba(239,68,68,0.3)', lineWidth: 1, lineStyle: 2, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: 'rgba(34,197,94,0.3)', lineWidth: 1, lineStyle: 2, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
    }

    if (indicator.type === 'STOCH_RSI') {
      series.push(chart.addLineSeries({ color: indicator.color, lineWidth: (indicator.lineWidth ?? 1) as 1|2|3|4, lineStyle: toLWLineStyle(indicator.lineStyle), priceLineVisible: false, lastValueVisible: true, crosshairMarkerVisible: true }));
      series.push(chart.addLineSeries({ color: indicator.color2 ?? '#FF5722', lineWidth: (indicator.lineWidth ?? 1) as 1|2|3|4, lineStyle: toLWLineStyle(indicator.lineStyle), priceLineVisible: false, lastValueVisible: true, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: 'rgba(239,68,68,0.3)', lineWidth: 1, lineStyle: 2, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: 'rgba(34,197,94,0.3)', lineWidth: 1, lineStyle: 2, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
    }
    ...
    if (indicator.type === 'MACD') {
      ...
      series.push(hist);
      series.push(chart.addLineSeries({ color: 'rgba(107,114,128,0.3)', lineWidth: 1, lineStyle: 2, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
    }

    if (indicator.type === 'ADX') {
      series.push(chart.addLineSeries({ color: indicator.color, lineWidth: (indicator.lineWidth ?? 2) as 1|2|3|4, lineStyle: toLWLineStyle(indicator.lineStyle), priceLineVisible: false, lastValueVisible: true, crosshairMarkerVisible: true }));
      series.push(chart.addLineSeries({ color: '#22c55e', lineWidth: 1, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: '#ef4444', lineWidth: 1, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: 'rgba(107,114,128,0.3)', lineWidth: 1, lineStyle: 2, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: 'rgba(107,114,128,0.3)', lineWidth: 1, lineStyle: 2, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
    }
    ...
    if (indicator.type === 'PCT_DIFF_DON') {
      series.push(chart.addLineSeries({ color: indicator.color, lineWidth: 2 as 1|2|3|4, priceLineVisible: false, lastValueVisible: true, crosshairMarkerVisible: true }));
      series.push(chart.addLineSeries({ color: '#ffffff', lineWidth: 2 as 1|2|3|4, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: '#FF6D00', lineWidth: 1 as 1|2|3|4, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: 'rgba(255,255,255,0.4)', lineWidth: 1 as 1|2|3|4, lineStyle: 2, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: 'rgba(255,255,255,0.4)', lineWidth: 1 as 1|2|3|4, lineStyle: 2, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: 'rgba(255,255,255,0.2)', lineWidth: 1 as 1|2|3|4, lineStyle: 1, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: 'rgba(255,255,255,0.2)', lineWidth: 1 as 1|2|3|4, lineStyle: 1, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
      series.push(chart.addLineSeries({ color: 'rgba(107,114,128,0.3)', lineWidth: 1, lineStyle: 2, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false }));
    }
  ...
  useEffect(() => {
    ...
    if (indicator.type === 'PCT_DIFF_DON') {
      ...
      if (pctDiff.length > 0) {
        seriesRefs.current[0]?.setData(pctDiff.map(toLD) as LineData[]);
  ...
  const label = indicator.type === 'STOCH_RSI' ? `StochRSI(${indicator.period})` :
    ...
    indicator.type === 'PCT_DIFF_DON' ? `%Diff Don(${indicator.period})` :
  ...
  const handleResizeStart = useCallback((e: React.MouseEvent) => {
    ...
    const handleMove = (ev: MouseEvent) => {
      ...
      setHeight(Math.max(40, Math.min(200, startHeight.current + delta)));
    };
    const handleUp = () => {
  ...
  return (
    <div className="relative shrink" style={{ height, minHeight: 40, maxHeight: '35%' }}>
      {/* Resize handle */}
      <div
      ...
      />
      <div className="absolute top-1 left-2 z-10 flex items-center gap-1.5 text-[9px] font-mono text-muted-foreground pointer-events-auto">
        <span style={{ color: indicator.color }}>{label}</span>
        <button
          onClick={() => removePanelIndicator(panelIndex, indicator.id)}
